...
public class Score {
    ...
    private static int strikeZone(List<Ball> playerBalls, List<Ball> randomBalls) {
        ...
        for (int i = 0; i < playerBalls.size(); i++) {
            count += addStrikeCount(playerBalls.get(i), randomBalls.get(i));
        }
        return count;
    }

    private static int addStrikeCount(Ball playerBall, Ball randomBall) {
    ...
    private static int ballZone(List<Ball> playerBalls, List<Ball> randomBalls) {
        int count = 0;
        for (int i = 0; i < playerBalls.size(); i++) {
            count += addBallCount(playerBalls.get(i), randomBalls.get(i), randomBalls);
        }
        return count;
    }

    private static int addBallCount(Ball playerBall, Ball randomBall, List<Ball> randomBalls) {
        if (!playerBall.equals(randomBall)
